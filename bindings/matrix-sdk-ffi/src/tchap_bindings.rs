use matrix_sdk_tchap::get_instance_from_email::{
    TchapGetInstance, TchapGetInstanceConfig, TchapGetInstanceError,
};
use thiserror::Error;
...
#[uniffi::export]
pub fn tchap_get_instance(
    config: &TchapGetInstanceConfig,
    for_email: String,
) -> Result<String, TchapGetInstanceErrorBridged> {
...
mod tests {
    ...
    fn test_tchap_get_instance() {
        ...
        assert_eq!(
            ...
            tchap_get_instance(&config, "testeur@dinum.tchap.beta.gouv.fr".to_string()).unwrap()
        );
        assert_eq!(
            "agent.externe.tchap.gouv.fr",
            tchap_get_instance(&config, "testeur@gmail.com".to_string()).unwrap()
        );
    }
}
