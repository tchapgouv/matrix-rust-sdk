...
impl Client {
    ...
    /// let client = Client::new(homeserver).await.unwrap();
    /// assert!(client.create_room_with_federated(request).await.is_ok());
    /// # };
    /// ```
    ...
    pub(self) fn add_acl_initial_state_event(
        ...
        request.initial_state.push(
            InitialStateEvent::with_empty_state_key(RoomServerAclEventContent::new(
                false,
...
pub(crate) mod tests {
    ...
    async fn test_await_room_remote_echo_will_timeout_if_room_is_found_but_not_synced() {
        ...
        let room = client
            ...
            //     is_direct: false,
            // }))
            .create_room_with_federated(assign!(CreateRoomRequest::new(), {
                invite: vec![],
                is_direct: false,
            }))
            // end BWI-specific
