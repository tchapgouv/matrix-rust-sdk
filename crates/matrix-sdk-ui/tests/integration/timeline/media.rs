...
use matrix_sdk_test::{ALICE, JoinedRoomBuilder, async_test, event_factory::EventFactory, TestResult};
...
async fn test_send_attachment_from_file() -> TestResult {
    ...
    let mut config = AttachmentConfig {
...
async fn test_send_attachment_from_bytes() -> TestResult {
    ...
    // BWI-specific
    config.info = Some(AttachmentInfo::File(BaseFileInfo { size: Some(uint!(42)), ..Default::default() }));
    // end BWI-specific
...
async fn test_send_attachment_to_big_should_return_error() {
    ...
    // Store a file in a temporary directory.
    let (_tmp_dir, file_path) = create_temporary_file("test.bin").unwrap();
